...
import type { PaperSchema } from '../data/schemas';
		...
		<Link href={`/dashboard/papers/${paper.id}`}>
			<Card className="cursor-pointer transition-colors hover:bg-gray-100">
				<CardHeader className="flex flex-row items-center justify-between gap-2">
					<CardTitle className="min-w-0 flex-1 truncate">{paper.title}</CardTitle>
					<IconComponent className={`h-5 w-5 shrink-0 ${statusIcon.colorClass}`} />
				</CardHeader>
				<CardContent>
